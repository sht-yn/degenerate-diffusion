...
#         for s, d in zip(subs, shape):
#             if s in dim_map:
#                 assert dim_map[s] == d, f"Inconsistent dimension for subscript '{s}'"
#             else:
#                 dim_map[s] = d
#                 dim_order.append(s)
...
#         # 各入力テンソルから該当インデックスを取り出して積を計算
#         term = 1
#         for subs, op in zip(input_subs, operands):
...
def einsum_sympy(subscripts, *operands):
    ...
    for subs, shape in zip(input_subs, shapes):
        ...
        for s, d in zip(subs, shape):
            if s in dim_map:
                if dim_map[s] != d:
                    raise ValueError(f"Inconsistent dimension for subscript '{s}': {dim_map[s]} vs {d}")
            else:
                dim_map[s] = d
                dim_order.append(s)
